...
- (void)setBaseView {
    ...
    UIView *selectView = [[UIView alloc] initWithFrame:CGRectMake(0, height - (height - 100), width, 30)];
    ...
    self.pickView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, height - (height - 130) , width,  height - 200)];
...
- (void)dateEnsureAction {
    if (self.delegate && [self.delegate respondsToSelector:@selector(JSAddressPickerRerurnBlockWithProvince:city:)]) {
        [self.delegate JSAddressPickerRerurnBlockWithProvince:self.province city:self.city];
    }
}
