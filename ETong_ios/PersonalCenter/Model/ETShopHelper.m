...
-(void)sendGoodsWithUserid:(NSString *)goodsid success:(ETResponseBlock)success faild:(ETResponseErrorBlock)faild{
    NSDictionary *para = @{@"a":@"DeliverOrder",@"id":goodsid};
    [self.manager GET:kURL_HEAD parameters:para progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ETHttpModel *model = [ETHttpModel mj_objectWithKeyValues:responseObject];
        if ([model.status isEqualToString:@"success"]) {
            success(model.data);
...
// 获取商品的附加属性
-(void)GetGoodsAttributesInfoWithGoodsid:(NSString *)goodsid success:(ETResponseBlock)success faild:(ETResponseErrorBlock)faild{
    NSDictionary *para = @{@"a":@"GetGoodsPropertyList",@"goodsid":goodsid};
    [self.manager GET:kURL_HEAD parameters:para progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ETHttpModel *model = [ETHttpModel mj_objectWithKeyValues:responseObject];
        if ([model.status isEqualToString:@"success"]) {
            NSArray *models = [AttributesModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
            success(@{@"goods":models});
        }else{
            faild(@"", nil);
        }
...
// 获取附加属性
-(void)GetGoodsAttributesInfoWithType:(NSString *)goodstype success:(ETResponseBlock)success faild:(ETResponseErrorBlock)faild{
    NSDictionary *para = @{@"a":@"GetGoodPropertyList",@"goodstype":goodstype};
    [self.manager GET:kURL_HEAD parameters:para progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ETHttpModel *model = [ETHttpModel mj_objectWithKeyValues:responseObject];
        if ([model.status isEqualToString:@"success"]) {
            NSArray *models = [AttributesModel mj_objectArrayWithKeyValuesArray:responseObject[@"data"]];
...
-(void)GetSortInfoWithType:(NSString *)type success:(ResponseBlock)success faild:(ETResponseErrorBlock)faild{
    NSDictionary *para = @{@"a":@"getShopTypeList",@"type":type};
    [self.manager GET:kURL_HEAD parameters:para progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        HttpModel *model = [HttpModel mj_objectWithKeyValues:responseObject];
        if ([model.status isEqualToString:@"success"]) {
            success(model.data);
