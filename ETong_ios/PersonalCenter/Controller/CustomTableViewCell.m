...
-(void)setFrameModel:(modelFrame *)frameModel{
    ...
    if (frameModel.myself) {
        ...
        NSURL *a = [NSURL URLWithString: [NSString stringWithFormat:@"%@%@", kIMAGE_URL_HEAD,self.selfPhoto]];
    
        [imageView sd_setImageWithURL:a placeholderImage:[UIImage imageNamed:@"girl"]options:SDWebImageRetryFailed];
    }else{
        
        
        NSURL *a = [NSURL URLWithString: [NSString stringWithFormat:@"%@%@", kIMAGE_URL_HEAD,self.otherPhoto]];
